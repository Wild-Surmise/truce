...
pub mod au3 {
    ...
    pub struct AppexPlistValues<'a> {
        ...
        /// `GarageBand` shows its expand affordance) and a
        /// `"size:{w,h}"` first-open hint (read by AUM); the macOS
        /// path passes `&[]`. Each entry renders as its own
        /// `<string>…</string>` line inside the `tags` array.
    ...
    pub fn render_appex_info_plist(values: &AppexPlistValues<'_>) -> String {
        ...
        // in the template (24 spaces). Empty when no extras, leaving
        // the `tags` array with just the category entry.
    ...
    mod tests {
        ...
        #[test]
        fn ios_render_emits_resizable_and_size_tags() {
            // The `resizable` tag is what makes GarageBand show its
            // expand affordance; `size:{w,h}` is AUM's first-open size.
            // Both must land as their own entries in the AudioComponents
            // `tags` array, alongside the category tag.
            let plist = render_appex_info_plist(&ios_values());
