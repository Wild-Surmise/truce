...
class AudioUnitFactory: AUViewController, AUAudioUnitFactory {
    ...
    private func setupGUIIfReady() {
        ...
        #if os(iOS)
        // Fit the editor to the host's safe-area frame so its
        // responsive layout reflows to the real device viewport.
        fitGUIToSafeArea()
        #else
    ...
    override func viewDidLayoutSubviews() {
        ...
        // iOS always fits to the host's safe-area frame - unlike the
        // macOS `didInitialLayout` skip (which avoids Logic stretching
        // the desktop editor on first layout), fitting on the very
        // first layout is the whole point on iOS: the host hands us a
        // pane sized to the device and we reflow into it immediately.
        fitGUIToSafeArea()
    }
    #endif
    ...
    #if os(iOS)
    /// Fit the editor to the host plug-in pane's safe-area frame.
    /// AUv3 hosts (GarageBand, AUM, Logic for iPad) hand us a UIView
    /// whose bounds track their pane; we drive the Rust editor to the
    /// safe-area size via `gui_set_size` and pin the container inside
    /// the safe-area insets so the editor's responsive layout reflows
    /// to the real device viewport instead of sitting at its built
    /// portrait size. When the editor opted out of resize
    /// (`gui_can_resize == 0`) we only position the natural-size
    /// container, matching the desktop `centerGUI` behaviour.
    private func fitGUIToSafeArea() {
        ...
        // home-indicator insets; fall back to raw bounds before the
        // safe area resolves (it is zero until the view is in a
        // window hierarchy).
        ...
        if let ctx = myCtx, let cb = g_callbacks,
           ...
           (hostW, hostH) != (guiPtSize.width, guiPtSize.height) {
            ...
            // differ from what we asked for. Position the container to
            // the clamped size, not the request.
            var actW: UInt32 = 0, actH: UInt32 = 0
            cb.pointee.gui_get_size(ctx, &actW, &actH)
            guiPtSize = NSSize(width: CGFloat(max(1, actW)),
                               height: CGFloat(max(1, actH)))
        ...
        // UIKit origin is top-left: minX / minY place the editor
