...
typedef struct {
    ...
    /* Host/UI/state-thread only. Returns 1 on success, 0 when the
     * preset number is unknown or the plugin has no factory preset
     * support. */
    int32_t (*factory_preset_load)(void *ctx, int32_t number);
...
extern const AuFactoryPresetDescriptor *g_factory_preset_descriptors;
extern uint32_t g_num_factory_presets;
