...
/// Stash the true OS window size in egui ctx data so plugin UI code
/// can read it back via [`actual_window_size`].
pub fn set_actual_window_size(ctx: &egui::Context, size: (u32, u32)) {
    ctx.data_mut(|data| data.insert_temp(actual_window_size_id(), size));
}

/// The true OS window size as stored by the editor each frame.
#[must_use]
