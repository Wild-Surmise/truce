...
pub struct EguiEditor<P: Params + ?Sized> {
    ...
    /// AUv3 view controller only fits the editor to the host's
...
impl<P: Params + 'static> EguiEditor<P> {
    ...
    /// Opt into host-driven resizing. When `true`, the AUv3 view
    ...
    /// desktop `EguiEditor::resizable` so the same builder call
    ...
    /// Minimum logical-point size the editor accepts. The AU shim's
    /// `clamp_logical_to_editor` consults this before driving
    /// `set_size`. See [`Self::resizable`].
    #[must_use]
...
impl<P: Params + 'static> Editor for EguiEditor<P> {
    ...
    fn set_size(&mut self, width: u32, height: u32) -> bool {
        ...
        // view + surface inline here is safe (no reentrancy with the
        // tick - they never nest).
...
fn run_frame<P: Params + 'static>(inner: &mut Inner<P>) {
    // Publish the true editor size out-of-band so the shared shell /
    // scaling logic reads it instead of egui's `screen_rect` (which
    // egui transiently rescales on zoom-change frames). Matches the
    // desktop editor's per-frame `set_actual_window_size`.
...
/// Re-size the live editor surface to `logical_w x logical_h`
/// logical points. Updates the cached logical size, the UIView
/// frame, the `CAMetalLayer` drawable (in physical pixels), and the
/// wgpu surface, then republishes the actual window size so the
/// next `run_frame`'s `screen_rect` reflows the egui layout. No-op
/// when the size is unchanged so redundant layout passes are cheap.
